...
/** \ingroup docGUI*/
//! NotePropertiesEditor is (currently) a single class instantiated in different
//! "modes" to select which property it edits. There are individual instances
//! for each property which are hidden and shown depending on what the user
//! selects.
class NotePropertiesRuler : public PatternEditor,
   ...
   public:
	...
	static void yToKeyOctave( int nY, H2Core::Note::Key* pKey, int* pOctave );
	...
	QPoint gridPointToPoint( const H2Core::GridPoint& gridPoint
	) const override;
   ...
   private:
	...
	//! gesture.
	std::map<std::shared_ptr<H2Core::Note>, std::shared_ptr<H2Core::Note> >
		m_oldNotes;
