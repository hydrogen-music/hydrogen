...
class PatternEditorPanel : public QWidget,
   ...
   public:
	...
	virtual void stateChangedEvent( const H2Core::AudioEngine::State& )
		override;
	...
	int findRowDB( std::shared_ptr<H2Core::Note> pNote, bool bSilent = false )
		const;
	 ...
	 */
	static bool isUsingAdditionalPatterns(
...
inline std::vector<std::shared_ptr<H2Core::Pattern>>
PatternEditorPanel::getPatternsToShow() const
{
