...
/**
 * Edit the properties of arbitrarily many notes via the
 * NotePropertiesRuler. It will be used to undo and redo the changes
 * applied to the individual notes.
 *
 * Whenever the user is using the mouse wheel or is moving the mouse
 * after pressing its left button, either the function
 * NotePropertiesRuler::wheelEvent or
 * NotePropertiesRuler::mousePressEvent will be triggered and will
 * change the properties of the corresponding notes according to the
 * input. To be able to revert all changes done within Hydrogen, an
 * SE_editNotePropertiesAction event will be generated by the
 * NotePropertiesRuler::pushUndoAction function and pushed onto the
 * QUndoStack. This event will be set up in such a way it can undo all
 * changes done via the user input.
 *
 * The actual resetting of the parameter during the undo/redo will be
 * done by the DrumPatternEditor::undoRedoNotePropertiesEditAction
 * function. Depending whether an undo or redo is required,
 * SE_editNotePropertiesAction will call the function using either to
...
public:
	/**
	 * Constructor.
	 * \param propertyChangesStack A list of all changes done by
	 * the last action. If the properties of just a single note
	...
	/**
	 * If the event is used to undo the last action, both a
	 * QString specifying the property, which was changed, and a
	 * struct containing the state of the note before the action
	 * will be used to call
	 * DrumPatternEditor::undoRedoNotePropertiesEditAction. 
	 */
	virtual void undo(){
		HydrogenApp* h2app = HydrogenApp::get_instance();
	 ...
	 * QString specifying the property, which was changed, and a
	 * struct containing the state of the note after the action
	 * will be used to call
	 * DrumPatternEditor::undoRedoNotePropertiesEditAction.
	 */
	virtual void redo(){
...
private:
	/**
	 * A list of all changes done by the last action. If the
	 * properties of just a single note were altered, the list
