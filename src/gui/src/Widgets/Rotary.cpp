...
void Rotary::paintEvent( QPaintEvent* ev )
{
	...
	fArcLineWidth = 2.0;
	rectArc = QRect(
		( m_nWidgetWidth - m_nWidgetHeight + fArcLineWidth ) * 0.5,
		fArcLineWidth / 2, m_nWidgetHeight - fArcLineWidth,
		m_nWidgetHeight - fArcLineWidth
	);
	rectRotary = QRect(
		rectArc.x() + fArcLineWidth / 2, rectArc.y() + fArcLineWidth / 2,
		rectArc.width() - fArcLineWidth, rectArc.height() - fArcLineWidth
	);
	...
	if ( m_bEntered || hasFocus() ) {
		painter.fillRect(
			0, m_nWidgetHeight - 2, m_nWidgetWidth, 2, colorHighlightActive
		);
	}
	...
	if ( m_bIsActive ) {
		if ( m_type == Type::Normal ) {
			...
			int nSpanAngle = static_cast<int>(
				-239 * 16 * ( m_fValue - m_fMin ) / ( m_fMax - m_fMin )
			);
			...
			if ( ( m_fValue - 0.5 * ( m_fMax + m_fMin ) ) == 0 ) {
				...
				painter.setPen(
					QPen( colorArcCenterUnset, fArcLineWidth * 1.25 )
				);
				painter.drawArc( rectArc, 91 * 16, -3 * 16 );

				int nStartAngle = -18 * 16;
				int nSpanAngle = static_cast<int>(
					-200 * 16 * ( m_fValue - 0.5 * ( m_fMax + m_fMin ) ) /
					( m_fMax - m_fMin )
				);
				if ( m_fValue - 0.5 * ( m_fMax + m_fMin ) < 0 ) {
					nStartAngle *= -1;
	...
	if ( m_type == Type::Center ) {
		...
		fCurrentAngle =
			fStartAngle + 255 * M_PI / 180 *
							  ( m_fValue - 0.5 * ( m_fMax + m_fMin ) ) /
							  ( m_fMax - m_fMin );
	}
	else {
		fStartAngle = -90 * M_PI / 180;
		fCurrentAngle = fStartAngle +
						255 * M_PI / 180 *
							( m_fValue - m_fMin - 0.5 * ( m_fMax - m_fMin ) ) /
							( m_fMax - m_fMin );
	}
	...
	if ( m_bIsActive ) {
		const float fBaseX = rectRotary.x() + rectRotary.width() / 2;
		const float fBaseY = rectRotary.y() + rectRotary.height() / 2;
		const float fDistance = 4;
		const float fRadius = 1.5;

		QPointF p1(
			fBaseX + std::cos( fCurrentAngle ) * fDistance,
	...
	if ( std::fmod( m_fMin, 1 ) == 0 && std::fabs( m_fMin ) < 10 ) {
		...
		painter.drawText(
			leftTextRec, Qt::AlignCenter, QString::number( m_fMin )
		);
	}
	else {
		font.setPixelSize( 9 );
	...
	if ( std::fmod( m_fMax, 1 ) == 0 && std::fabs( m_fMax ) < 10 ) {
		...
		painter.setFont( font );
		painter.drawText(
			rightTextRec, Qt::AlignCenter, QString::number( m_fMax )
		);
	}
	else {
		font.setPixelSize( 9 );
