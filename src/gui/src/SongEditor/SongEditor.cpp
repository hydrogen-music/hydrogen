...
void SongEditor::keyPressEvent( QKeyEvent * ev )
{
	...
	H2Core::Song::ActionMode actionMode = pEngine->getSong()->getActionMode();
	...
	if ( ev->matches( QKeySequence::Delete ) ) {
		...
		} else if ( actionMode == H2Core::Song::ActionMode::selectMode ) {
...
{
	if ( !m_bShowLasso
		 && Hydrogen::get_instance()->getSong()->getActionMode() == H2Core::Song::ActionMode::selectMode ) {
		QPoint pos = (columnRowToXy( QPoint( m_nCursorColumn, m_nCursorRow ) )
					  + QPoint( m_nGridWidth / 2, m_nGridHeight / 2) );
		m_bShowLasso = true;
		m_bIsMoving = false;
...
void SongEditor::mouseMoveEvent(QMouseEvent *ev)
{
	...
	H2Core::Song::ActionMode actionMode = Hydrogen::get_instance()->getSong()->getActionMode();
	...
	if ( !m_bDragging ) {
		if ( (ev->pos() - m_clickStartPoint).manhattanLength()
			...
			} else if ( actionMode == H2Core::Song::ActionMode::drawMode ) {
	...
	if ( m_bDragging ) {
		...
		} else if ( Hydrogen::get_instance()->getSong()->getActionMode() == H2Core::Song::ActionMode::drawMode ) {
...
void SongEditor::mouseReleaseEvent( QMouseEvent *ev )
{
	...
	if ( m_bDragging ) {
		...
		} else if ( Hydrogen::get_instance()->getSong()->getActionMode() == H2Core::Song::ActionMode::selectMode ) {
			if ( m_bIsCtrlPressed ) {
