...
// ZOOM
	...
	InstrumentComponent* pCompo = Hydrogen::get_instance()->getAudioEngine()->get_sampler()->getPlaybackTrackInstrument()->get_components()->front();
	assert(pCompo);
...
{
	Sampler*		pSampler = Hydrogen::get_instance()->getAudioEngine()->get_sampler();
	Preferences *	pPref = Preferences::get_instance();
