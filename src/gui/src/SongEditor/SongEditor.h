...
class SongEditorPatternList :  public QWidget, protected WidgetWithScalableFont<8, 10, 12>,  public H2Core::Object<SongEditorPatternList>, public EventListener
{
	 ...
	 * highlighting.
	 *
	 * The highlighting starts when the user right-clicks a
	 * row/pattern name and the associated popup window is shown. This
	 * will set #m_rowSelection to RowSelection::Popup. As soon as
	 * the popup get's hidden (by clicking at an arbitrary position),
	 * it will be reset to RowSelection::None by a callback. If the
	 * user clicks an action of the popup, #m_rowSelection will be
	 * upgraded to RowSelection::Dialog and set the RowSelection::None
	 * once the associated dialog is closed.
	 *
	...
	/**
	 * Helper variable remembering for which pattern index/row was
	 * right-clicked last.
	 */
	int m_nRowClicked;
