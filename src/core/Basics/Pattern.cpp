...
QString Pattern::toQString( const QString& sPrefix ) const {
	QString s = Logger::printIndention;
	QString sOutput = QString( "%1[Pattern]\n" ).arg( sPrefix )
	...
	for ( auto it = __notes.begin(); it != __notes.end(); it++ ) {
		if ( it->second != nullptr ) {
			sOutput.append( QString( "%1" ).arg( it->second->toQString( sPrefix + s + s ) ) );
		}
	}
	...
	for ( auto ii : __virtual_patterns ) {
		if ( ii != nullptr ) {
			sOutput.append( QString( "%1" ).arg( ii->toQString( sPrefix + s + s ) ) );
		}
	}
...
std::ostream& operator<<( std::ostream& os, const Pattern& pattern ) {
	return os << pattern.toQString( "" ).toLocal8Bit().data() << std::endl;
}
std::ostream& operator<<( std::ostream& os, const Pattern* pattern ) {
	return os << pattern->toQString( "" ).toLocal8Bit().data() << std::endl;
}
