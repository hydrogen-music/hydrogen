	  ...
	  m_fLastSampleLoadBpm( MIN_BPM )
{
	...
	const QString sInstrumentName =
		QT_TRANSLATE_NOOP( "Instrument", "New Instrument" );
	  ...
	  m_pComponents( nullptr )
{
	...
	for ( const auto& pComponent : *other->getComponents() ) {
		m_pComponents->push_back(
			std::make_shared<InstrumentComponent>( pComponent )
		);
	}
}
...
{
	if ( m_nQueued > 0 ) {
		WARNINGLOG( QString( "Instrument [%1] is destroyed while still being "
							 "enqueued! m_nQueued: %2,\nm_enqueuedNotes:\n\t%3"
		)
						.arg( m_sName )
						.arg( m_nQueued )
...
)
{
	...
	if ( flags & Xml::Flag::SongKit && ! ( flags & Xml::Flag::Project ) ) {
		...
		sInstrumentDrumkitName =
			node.read_string( "drumkit", "", false, true, bSilent );
				...
				if ( sInstrumentDrumkitPath.isEmpty() && nLookup != 0 ) {
					...
					if ( sInstrumentDrumkitPath.isEmpty() ) {
						ERRORLOG( QString( "Drumkit [%1] could neither found "
										   "at system nor at user level." )
									  .arg( sInstrumentDrumkitName ) );
					}
					else if ( !bSilent ) {
						WARNINGLOG(
							QString( "Drumkit [%1] could not found using "
	...
	pInstrument->setVolume(
		node.read_float( "volume", 1.0f, true, true, bSilent )
	);
	pInstrument->setMuted(
		node.read_bool( "isMuted", false, true, true, bSilent )
	);
	pInstrument->setSoloed(
		node.read_bool( "isSoloed", false, true, false, true )
	);
	bool bFound, bFound2;
	float fPan =
		node.read_float( "pan", PAN_DEFAULT, &bFound, true, true, true );
	if ( !bFound ) {
		// check if pan is expressed in the old fashion (version <=
		// 1.1 ) with the pair (pan_L, pan_R)
		float fPanL =
			node.read_float( "pan_L", 1.f, &bFound, true, true, bSilent );
		float fPanR =
			node.read_float( "pan_R", 1.f, &bFound2, true, true, bSilent );
		if ( bFound == true &&
			 bFound2 == true ) {  // found nodes pan_L and pan_R
	...
	pInstrument->setApplyVelocity(
		node.read_bool( "applyVelocity", true, false, true, bSilent )
	);
	pInstrument->setFilterActive(
		node.read_bool( "filterActive", true, false, true, bSilent )
	);
	pInstrument->setFilterCutoff(
		node.read_float( "filterCutoff", 1.0f, true, false, bSilent )
	);
	pInstrument->setFilterResonance(
		node.read_float( "filterResonance", 0.0f, true, false, bSilent )
	);
	pInstrument->setPitchOffset(
		node.read_float( "pitchOffset", 0.0f, true, false, true )
	);
	pInstrument->setRandomPitchFactor(
		node.read_float( "randomPitchFactor", 0.0f, true, false, bSilent )
	);
	pInstrument->setGain( node.read_float( "gain", 1.0f, true, false, bSilent )
	);
	...
	if ( license == License() ) {
		...
		if ( pSoundLibraryDatabase != nullptr &&
			...
			auto pDrumkit =
				pSoundLibraryDatabase->getDrumkit( pInstrument->getDrumkitPath()
				);
			if ( pDrumkit != nullptr ) {
	...
	if ( !node.firstChildElement( "instrumentComponent" ).isNull() ) {
		...
		while ( !componentNode.isNull() ) {
			...
			componentNode =
				componentNode.nextSiblingElement( "instrumentComponent" );
		}
	}
	else {
	...
	if ( !bShort ) {
		...
		sOutput =
			QString( "[Instrument]" )
				...
				.append(
					QString( ", m_bFilterActive: %1" ).arg( m_bFilterActive )
				)
				.append(
					QString( ", m_fFilterCutoff: %1" ).arg( m_fFilterCutoff )
				)
				.append( QString( ", m_fFilterResonance: %1" )
							 .arg( m_fFilterResonance ) )
				.append( QString( ", m_fRandomPitchFactor: %1" )
							 .arg( m_fRandomPitchFactor ) )
				.append( QString( ", m_fPitchOffset: %1" ).arg( m_fPitchOffset )
				)
				.append( QString( ", m_midiOutNote: %1" )
							 .arg( static_cast<int>( m_midiOutNote ) ) )
				...
				.append( QString( ", m_bIsPreviewInstrument: %1" )
							 .arg( m_bIsPreviewInstrument ) )
				.append(
					QString( ", m_bApplyVelocity: %1" ).arg( m_bApplyVelocity )
				)
				.append( QString( ", m_bCurrentInstrForExport: %1" )
							 .arg( m_bCurrentInstrForExport ) )
				.append( QString( ", m_bHasMissingSamples: %1" )
							 .arg( m_bHasMissingSamples ) )
