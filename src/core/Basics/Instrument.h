...
class Instrument : public H2Core::Object<Instrument> {
   ...
   public:
	...
	float getPan() const;
	/** get pan of the instrument scaling and translating the range from [-1;1]
	 * to [0;1] */
   ...
   private:
	...
	float m_fPan;	  ///< pan of the instrument, [-1;1] from left to right, as
					  ///< requested by Sampler PanLaws
	...
	bool m_bCurrentInstrForExport;	///< is the instrument currently being
									///< exported?
	bool m_bHasMissingSamples;		///< does the instrument have missing sample
									///< files?
	std::shared_ptr<std::vector<std::shared_ptr<InstrumentComponent>>>
		m_pComponents;
};
...
inline void Instrument::setPan( float val
)  // TODO check boundary factorize function?
{
	if ( val > 1.0 ) {
		m_fPan = 1.0;
	}
...
inline const QString& Instrument::getDrumkitName() const
{
...
inline std::shared_ptr<std::vector<std::shared_ptr<InstrumentComponent>>>
Instrument::getComponents() const
{
