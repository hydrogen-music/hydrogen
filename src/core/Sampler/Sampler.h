...
class Sampler : public H2Core::Object<Sampler> {
   ...
   public:
	/** PAN LAWS
	 * The following pan law functions return pan_L (==L, which is the gain for
	 *Left channel). They assume the fPan argument domain = [-1;1], which is
	 *used in Note and Instrument classes.
	 *----------------------------
	 * For the right channel use: R(p) == pan_R(p) = pan_L(-p) == L(-p)
	 * thanks to the Left-Right symmetry.
	 *--------------------------------------
	 * The prefix of the function name tells the interpretation of the fPan
	 *argument:
	 *
	 * "ratio" parameter:
	 ...
	 *
	 * Note: using a different fPan interpretation makes the output signal more
	 *central or more lateral. From more central to more lateral: "quadratic"
	 *---> "ratio" ---> "polar" ---> "linear"
	 *---------------------------------------------
	 * After the prefix, the name describes the Image of the pan law in the LR
	 *plane. (remember that each pan law is a parametrized curve in LR plane.
	 * E.g.:
	 *	"ConstantSum":
	 *		it's properly used in an anechoic room, where there are no
	 *reflections. Ensures uniform volumes in MONO export, has -6.02 dB center
	 *compensation. "ConstantPower": probably makes uniform volumes in a common
	 *room, has -3.01 dB center compensation. "ConstantKNorm": L^k + R^k = const
	 ...
	 * Pan laws can be approximated by polynomials, e.g. with degree = 2, to
	 *adjust the center compensation, but then you cannot control the
	 *interpretation of the fPan argument exactly.
	 */
	enum PAN_LAW_TYPES {
	 ...
	 * as it was input from the GUI (up to scale and translation, which is
	 *arbitrary). Default output is 0 (=central pan) if arguments are invalid.
	 *-----Historical Note-----
	 * Originally (version <= 1.0) pan_L,pan_R were actually gains for each
	 *channel; "instrument" and "note" pans were multiplied as in a gain CHAIN
	 *in each separate channel, so the chain killed the signal if instrument and
	 *note pans were hard-sided to opposites sides! In v1.1, pan_L and pan_R
	 *were still the members of Note/Instrument representing the pan knob
	 *position, still using the ratioStraightPolygonalPanLaw() for the
	 *correspondence (up to constant multiplication), but pan_L,pan_R were
	 *reconverted to single parameter in the Sampler, and fPan was used in the
	 *selected pan law.
	 */
   ...
   private:
	...
	std::map<
		...
		std::shared_ptr<InstrumentLayer>>
		m_lastUsedLayersMap;
};

inline const std::vector<std::shared_ptr<Note>>& Sampler::getPlayingNotesQueue(
) const
{
