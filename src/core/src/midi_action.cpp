...
bool MidiActionManager::open_song(Action* pAction, Hydrogen* pHydrogen, targeted_element element) {
	...
	// // Check whether the provided path is valid.
	// if ( !isSongPathValid( songPath ) ) {
	// 	std::cerr << "Error: Song path [" << songPath.toLocal8Bit().data() 
	...
	// 	std::cerr << "Error: Selected Song does not exist!" << std::endl;
	// 	return false;
	// }
	...
	// if ( pSong == nullptr ) {
	// 	std::cerr << "Error: Unable to open Song." << std::endl;
...
bool MidiActionManager::save_all(Action* pAction, Hydrogen* pHydrogen, targeted_element element) {
	...
	// bool resultSaveDrumkit = save_drumkit( pAction, pHydrogen, element );
...
bool MidiActionManager::save_drumkit(Action* pAction, Hydrogen* pHydrogen, targeted_element element) {
...
bool MidiActionManager::save_song(Action* pAction, Hydrogen* pHydrogen, targeted_element element) {
	...
	if ( filename.isEmpty() ) {
		std::cerr << "Error: Unable to save Song. Empty filename! " 
...
bool MidiActionManager::quit(Action* pAction, Hydrogen* pHydrogen, targeted_element element) {
	...
	if ( pHydrogen->getActiveGUI() != 0 ) {
		...
		std::cerr << "Error: Closing the application via the core part is not supported yet!" <<
			std::endl;
...
bool MidiActionManager::isSongPathValid( const QString& songPath ) {
	...
	if ( !songFileInfo.isAbsolute() ) {
		std::cerr << "Error: Unable to handle path ["<<
