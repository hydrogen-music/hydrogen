...
class Filesystem : public H2Core::Object<Filesystem> {
   ...
   public:
	 ...
	 * drumkit.xml file \param path the path to search in for drumkits
	 */
	 ...
	 */
	static QString drumkit_path_search(
	 ...
	 * then system drumkits \param dk_name the drumkit name \param lookup Where
	 * to search (system/user folder or both) for the drumkit.
	 */
	static QString
	drumkit_dir_search( const QString& dk_name, const Lookup& lookup );
	/**
	 ...
	 * not) \param path the path to the file to check \param silent output not
	 * messages if set to true
	 */
	 ...
	 * create if it not exists \param path the path to the file to check \param
	 * create will try to create path if not exists and set to true \param
	 * silent output not messages if set to true
	 */
	static bool
	path_usable( const QString& path, bool create = true, bool silent = false );
	 ...
	 */
	static bool
	rm( const QString& path, bool recursive = false, bool bSilent = false );
	/**
	 * create a path
   ...
   private:
	 ...
	 */
	static bool
	check_permissions( const QString& path, const int perms, bool silent );
