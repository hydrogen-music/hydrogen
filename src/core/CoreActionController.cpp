	...
	const std::vector<std::shared_ptr<Note>>& selected )
{
	...
	auto pPattern = pPatternList->get( nPatternNumber );
	if ( pPattern == nullptr ) {
		ERRORLOG( QString( "Invalid pattern [%1]" ).arg( nPatternNumber ) );
		return false;
	}
...
bool CoreActionController::restoreOverwrittenNotes(
	int nPatternNumber,
	const std::vector<std::shared_ptr<Note>>& overwritten )
{
	...
	auto pAudioEngine = m_pHydrogen->getAudioEngine();
	pAudioEngine->lock( RIGHT_HERE );
	for ( const auto& pNote : overwritten ) {
		if ( pNote != nullptr ) {
			pPattern->insertNote( std::make_shared<Note>( pNote ) );
		}
	}
	pAudioEngine->unlock();
