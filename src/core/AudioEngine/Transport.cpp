...
void Transport::setBpm( float fNewBpm )
{
	if ( fNewBpm > MAX_BPM ) {
		...
		ERRORLOG( QString( "[%1] Provided bpm [%2] is too low. Assigning lower "
						   "bound %3 instead" )
					  .arg( TypeToQString( m_type ) )
					  .arg( fNewBpm )
					  .arg( MIN_BPM ) );
...
)
{
	...
	if ( pHydrogen->isTimelineEnabled() &&
		...
		auto handleEnd = [&]() {
			...
			if ( fRoundingErrorInTicks >
				...
				*fTickMismatch = fPassedTicks + fRemainingTicks - fNextTick;
				...
				const double fFinalFrame =
					fNewFrame + ( fNextTick - fPassedTicks - fRemainingTicks ) *
									fNextTickSize;
				...
				TP_DEBUGLOG(
					QString(
						...
						"[%1 + %2], static_cast<double>(nNewFrame): %3, "
						"fNewFrame: %4, fFinalFrame: %5, fNextTickSize: %6, "
						"fPassedTicks: %7, fRemainingTicks: %8, "
						"fFinalTickSize: %9"
					)
			...
			TP_DEBUGLOG(
				QString(
					...
					"fNextTick: %3, fRemainingTicks: %4, fPassedTicks: %5, "
					"fNextTickSize: %6, tempoMarkers[ ii - 1 ]->nColumn: %7, "
					"tempoMarkers[ ii - 1 ]->fBpm: %8, nNewFrame: %9, "
					"fTickMismatch: %10, frame increment (fRemainingTicks * "
					"fNextTickSize): %11, fRoundingErrorInTicks: %12"
				)
					.arg( fTick, 0, 'f' )
		...
		while ( fRemainingTicks > 0 ) {
			for ( ii = 1; ii <= tempoMarkers.size(); ++ii ) {
				if ( ii == tempoMarkers.size() ||
					...
					fNextTick = static_cast<double>(
						pHydrogen->getTickForColumn( tempoMarkers[ii]->nColumn )
					);
				}
				...
				if ( fRemainingTicks > ( fNextTick - fPassedTicks ) ) {
					...
					TP_DEBUGLOG(
						QString(
							...
							"%3, fRemainingTicks: %4, fPassedTicks: %5, "
							"fNextTickSize: %6, tempoMarkers[ ii - 1 "
							"]->nColumn: %7, tempoMarkers[ ii - 1 ]->fBpm: %8, "
							"tick increment (fNextTick - fPassedTicks): %9, "
							"frame increment (fRemainingTicks * "
							"fNextTickSize): %10"
						)
							.arg( fTick, 0, 'f' )
							.arg( fNewFrame, 0, 'g', 30 )
							...
							.arg(
								( fNextTick - fPassedTicks ) * fNextTickSize, 0,
								'g', 30
							)
					);
#endif
			...
			if ( fRemainingTicks > 0 ) {
				...
				TP_DEBUGLOG(
					QString( "[repeat] fTick: %1, fNewFrames: %2, fNewTick: "
							 ...
							 "fSongSizeInTicks: %6, fSongSizeInFrames: %7" )
						.arg( fTick, 0, 'g', 30 )
						.arg( fNewFrame, 0, 'g', 30 )
						.arg( fNewTick, 0, 'g', 30 )
				...
				if ( std::isinf( fNewFrame ) ||
					...
					ERRORLOG( QString( "Provided ticks [%1] are too large." )
								  .arg( fTick ) );
					return 0;
				}
				...
				if ( fRemainingTicks == 0 ) {
					...
					fNextTick = static_cast<double>(
						pHydrogen->getTickForColumn( tempoMarkers[0]->nColumn )
					);
					fNextTickSize = AudioEngine::computeDoubleTickSize(
						nSampleRate, tempoMarkers[ii - 1]->fBpm
					);
		...
		const double fNewFrame = static_cast<double>( fTick ) * fTickSize;
		nNewFrame = static_cast<long long>( std::round( fNewFrame ) );
		*fTickMismatch =
...
Transport::computeTickFromFrame( const long long nFrame, int nSampleRate )
{
	...
	if ( nFrame < 0 ) {
		ERRORLOG(
			QString( "Provided frame [%1] must be non-negative" ).arg( nFrame )
		);
	}
		...
		while ( fPassedFrames < fTargetFrame ) {
				...
				if ( fNextFrame < ( fTargetFrame - fPassedFrames ) ) {
					...
					TP_DEBUGLOG(
						QString(
							...
							"fNextTickSize: %4, fNextTicks: %5, fNextFrame: "
							"%6, tempoMarkers[ ii -1 ]->nColumn: %7, "
							"tempoMarkers[ ii -1 ]->fBpm: %8, fPassedTicks: "
							"%9, fPassedFrames: %10, fNewTick (tick "
							"increment): %11, fNewTick * fNextTickSize (frame "
							"increment): %12"
						)
							.arg( nFrame )
							.arg( fTick, 0, 'f' )
							...
							.arg(
								( fNextTicks - fPassedTicks ) * fNextTickSize,
								0, 'g', 30
							)
					);
#endif
					...
					const double fNewTick =
						( fTargetFrame - fPassedFrames ) / fNextTickSize;
					...
					TP_DEBUGLOG(
						QString( "[end] nFrame: %1, fTick: %2, nSampleRate: "
								 ...
								 "fNextFrame: %6, tempoMarkers[ ii -1 "
								 "]->nColumn: %7, tempoMarkers[ ii -1 ]->fBpm: "
								 "%8, fPassedTicks: %9, fPassedFrames: %10, "
								 "fNewTick (tick increment): %11, fNewTick * "
								 "fNextTickSize (frame increment): %12" )
							.arg( nFrame )
							.arg( fTick, 0, 'f' )
							.arg( nSampleRate )
							.arg( fNextTickSize, 0, 'f' )
			...
			if ( fPassedFrames != fTargetFrame ) {
				...
				const int nRepetitions =
					std::floor( fTargetFrame / fSongSizeInFrames );
				if ( fSongSizeInTicks * nRepetitions >
					 std::numeric_limits<double>::max() ) {
					ERRORLOG( QString( "Provided frames [%1] are too large." )
								  .arg( nFrame ) );
					return 0;
				}
				...
				TP_DEBUGLOG(
					QString( "[repeat] frames covered: %1, frames remaining: "
							 ...
							 "fSongSizeInFrames: %5, fSongSizeInTicks: %6" )
...
)
{
	...
	else if ( pLhs->m_pPlayingPatterns != nullptr && pRhs->m_pPlayingPatterns != nullptr && *pLhs->m_pPlayingPatterns != *pRhs->m_pPlayingPatterns ) {
		return false;
	}

	if ( ( pLhs->m_pNextPatterns != nullptr && pRhs->m_pNextPatterns == nullptr
		 ) ||
		 ( pLhs->m_pNextPatterns == nullptr && pRhs->m_pNextPatterns != nullptr
		 ) ) {
		return false;
	}
	else if ( pLhs->m_pNextPatterns != nullptr && pRhs->m_pNextPatterns != nullptr && *pLhs->m_pNextPatterns != *pRhs->m_pNextPatterns ) {
		return false;
	}
	...
	return (
		...
		std::abs( pLhs->m_fTickOffsetQueuing - pRhs->m_fTickOffsetQueuing ) <
			1E-5 &&
		std::abs( pLhs->m_fTickOffsetSongSize - pRhs->m_fTickOffsetSongSize ) <
			1E-5 &&
		pLhs->m_nPatternSize == pRhs->m_nPatternSize &&
		pLhs->m_nLastLeadLagFactor == pRhs->m_nLastLeadLagFactor &&
		pLhs->m_nBar == pRhs->m_nBar && pLhs->m_nBeat == pRhs->m_nBeat
	);
}
...
QString Transport::toQString( const QString& sPrefix, bool bShort ) const
{
	...
	if ( !bShort ) {
		...
		if ( m_pPlayingPatterns != nullptr ) {
			sOutput.append(
				QString( "%1%2m_pPlayingPatterns: %3\n" )
					.arg( sPrefix )
					.arg( s )
		...
		sOutput
			.append( QString( "%1%2m_nPatternSize: %3\n" )
						 .arg( sPrefix )
						 .arg( s )
						 .arg( m_nPatternSize ) )
		...
		sOutput =
			QString( "%1[Transport]" )
				...
				.append(
					QString( ", m_fTickSize: %1" ).arg( m_fTickSize, 0, 'f' )
				)
				.append( QString( ", m_fBpm: %1" ).arg( m_fBpm, 0, 'f' ) )
				.append( QString( ", m_nPatternStartTick: %1" )
							 .arg( m_nPatternStartTick ) )
				.append( QString( ", m_nPatternTickPosition: %1" )
							 .arg( m_nPatternTickPosition ) )
				.append( QString( ", m_nColumn: %1" ).arg( m_nColumn ) )
				.append( QString( ", m_fTickMismatch: %1" )
							 .arg( m_fTickMismatch, 0, 'f' ) )
				.append( QString( ", m_nFrameOffsetTempo: %1" )
							 .arg( m_nFrameOffsetTempo ) )
				.append( QString( ", m_fTickOffsetQueuing: %1" )
							 .arg( m_fTickOffsetQueuing, 0, 'f' ) )
				.append( QString( ", m_fTickOffsetSongSize: %1" )
							 .arg( m_fTickOffsetSongSize, 0, 'f' ) );
		if ( m_pNextPatterns != nullptr ) {
		...
		sOutput
			.append( QString( ", m_nPatternSize: %1" ).arg( m_nPatternSize ) )
			.append( QString( ", m_nLastLeadLagFactor: %1" )
						 .arg( m_nLastLeadLagFactor ) )
